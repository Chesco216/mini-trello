...
export const WorkspaceCard = ({ workspace }: Props) => {
  ...
  return (
    <div
      onClick={handleClick}
      className="cursor-pointer flex flex-col bg-lblue gap-2 rounded-xl overflow-hidden xl:w-md"
    >
      ...
      <span className="flex flex-col gap-5 p-5 bg-bgtgray">
        <label className="flex flex-col font-light font-semibold text-md ">{description}</label>
        <label className="flex flex-row gap-4">Colaborators: <MembersIcons members={workspace.members} /></label>
